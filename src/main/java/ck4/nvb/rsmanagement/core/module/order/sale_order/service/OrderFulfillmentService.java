...
public class OrderFulfillmentService {
  ...
  @Autowired private SaleAllocationRepository saleAllocationRepository;
  ...
  public void pickItemsForSaleLine(Long saleLineId, UserGetDto user) throws AppException {
    List<SaleAllocationDto> allocations =
        saleAllocationService.getAll(
            List.of(
                new SearchCriteria("saleLineId", SearchOperator.EQUALS, saleLineId.toString())));
    ...
    for (SaleAllocationDto allocation : allocations) {
      if (allocation.getQtyPicked() < allocation.getQtyAllocated()) {
        ...
        log.info(
            "Picked {} items from allocation {} for sale line {}",
  ...
  public void completeOrderFulfillment(String orderId, UserGetDto user) throws AppException {
    ...
    List<SaleAllocation> orderAllocations =
        saleAllocationRepository.findAllocationsByOrderId(orderId);
    ...
    for (SaleAllocation allocation : orderAllocations) {
      if (allocation.getQtyPicked() > 0) {
        ...
        log.info(
            "Completed fulfillment for allocation {}. Updated batch stock {}",
            allocation.getId(),
            batchStock.getId());
      }
    }
  ...
  public void cancelOrder(String orderId, UserGetDto user) throws AppException {
    ...
    for (SaleAllocation allocation : orderAllocations) {
      if (allocation.getQtyPicked() == 0) { // Only cancel unpicked allocations
        ...
        log.info(
            "Cancelled allocation {} and released {} units back to inventory",
            allocation.getId(),
            allocation.getQtyAllocated());
      }
    }
  ...
  @Transactional
  public void partialPick(Long allocationId, Integer qtyToPick, UserGetDto user)
      throws AppException {
  ...
  public OrderFulfillmentStatus getOrderFulfillmentStatus(String orderId) {
    ...
    boolean partiallyPicked =
        allocations.stream().anyMatch(allocation -> allocation.getQtyPicked() > 0);
