...
public class ProductFilterInputDto implements FilterInput {
    ...
    public List<SearchCriteria> mapToSearchCriteria() {
        ...
        if (minPrice != null) {
            criteria.add(new SearchCriteria("unitPrice", SearchOperator.GREATER_THAN_OR_EQUAL, minPrice.toString()));
        }

        if (maxPrice != null) {
            criteria.add(new SearchCriteria("unitPrice", SearchOperator.LESS_THAN_OR_EQUAL, maxPrice.toString()));
        }
