...
public class PaymentMethodServiceImpl
    extends FullAuditedCrudServiceImpl<PaymentMethodDto, PaymentMethod, Long, UserGetDto, Long> implements IPaymentService {
  ...
  public List<PaymentMethodGetDto.WithUsageStats> getUsageStatsOfInterval(int days) {
    ...
    return results.stream()
            .map(row -> new PaymentMethodGetDto.WithUsageStats(
                    (String) row.get("name"),
                    ((Number) row.get("paymentUsage")).intValue()
            )).toList();
  }
}
