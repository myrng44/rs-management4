...
public class SaleLineServiceImpl
  ...
  public SaleLineGetDto mapToEntityDto(SaleLine entity) {
    ...
    if (product == null) {
      throw new AppException("Product " + entity.getProductId() + " đã bị xóa hoặc không tồn tại.");
    }
  ...
  @Override
  public List<TopProductProjection> getMostSoldProductsLastDay(int days, int noProducts)
          throws AppException {
    LocalDateTime end = LocalDateTime.now();
    LocalDateTime start = end.minusDays(days);

    return getRepository().findMostSoldProductsOfInterval(start, end, noProducts);
  }
