...
public class JwtTokenServiceImpl implements JwtTokenService {
  ...
  public TokenResponseDto getToken(TokenRequestDto request) throws AppException {
    ...
    if (request.getStoreId() != null) {
      // login to a store
      userRole =
          userService.getByUsernameAndPasswordAndStore(
              request.getUsername(), request.getPassword(), request.getStoreId());
    } else {
      // login with primary role
      userRole = userService.getByUsernameAndPassword(request.getUsername(), request.getPassword());
    }

    logger.info(
        "[traceId-{}] User {}|{} - Get Access Token from IP: {} for Store: {}",
        request.getTraceId(),
        userRole.getUserId(),
  ...
  public TokenResponseDto refreshToken(TokenRefreshRequestDto request) throws AppException {
    ...
    if (refreshToken.getExpiredTime().isBefore(LocalDateTime.now())) {
      refreshTokenService.delete(
          refreshToken.getId(), createUserGetDto(refreshToken.getCreatorId()));
