...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<UserDTO>> getCurrentUser(HttpServletRequest request) {
        try {
            ...
            userDTO.setCreateAt(user.getCreatedTime());
            userDTO.setCreateBy(user.getCreatorId());
            userDTO.setUpdateAt(user.getUpdatedTime());
            userDTO.setUpdateBy(user.getUpdaterID());
    ...
    private Claims getClaims(String token) {
        ...
        return Jwts.parser()               // tạo parser
                .build()                   // build parser
                .parseSignedClaims(token)  // parse token có chữ ký
                .getPayload();
    }
}