...
public abstract class BaseSearchService<T extends BaseSearchDocument<ID>, ID> {
  ...
  @Autowired protected QueryBuilder queryBuilder;
  ...
  public List<String> getSuggestions(String prefix, String field) throws IOException {
    ...
    try {
      co.elastic.clients.elasticsearch.core.SearchResponse<T> resp =
          elasticsearchClient.search(
              s -> s.index(getIndexName()).query(query).size(10), documentClass);
      ...
      log.error(
          "Elasticsearch search failed — index={}, prefix={}, field={}, query={}",
          getIndexName(),
          prefix,
          field,
          query,
  ...
  public T indexDocument(T document) throws IOException {
    elasticsearchClient.index(
        i -> i.index(getIndexName()).id(document.getId().toString()).document(document));
    return document;
  }
  ...
  public void deleteById(ID id) throws IOException {
    elasticsearchClient.delete(d -> d.index(getIndexName()).id(id.toString()));
  }
  ...
  private SearchResponse<T> executeSearch(Query query, int page, int size) throws IOException {
    log.debug(
        "executeSearch index={}, page={}, size={}, query={}", getIndexName(), page, size, query);
    co.elastic.clients.elasticsearch.core.SearchResponse<T> resp =
        elasticsearchClient.search(
            s -> s.index(getIndexName()).query(query).from(Math.max(0, page * size)).size(size),
            documentClass);
    log.debug(
        "ElasticsearchResponse: hits={}, total={}",
        resp.hits().hits().size(),
        resp.hits() != null && resp.hits().total() != null
    ...
    return new SearchResponse<>(content, total, page, size, System.currentTimeMillis());
  }
}
