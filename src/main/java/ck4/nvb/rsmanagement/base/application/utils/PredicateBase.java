...
public class PredicateBase<T> {
     ...
     * @return          predicate
     */
    public BooleanExpression getPredicate(String key, SearchOperator operator, String value) throws Exception {
    ...
    private BooleanExpression getPredicate(String key, SearchOperator operator, String value, PathBuilder<?> entityPath,
        ...
        switch (propertyType.getSimpleName()){
            case "Integer": {
                ...
                if (isMultiValue) {
                    return getNumberPredicate(path, operator, Stream.of(value.split(multivalueSeperator)).map(Integer::parseInt).toArray(Integer[]::new));
                } else {
            ...
            case "Long": {
                ...
                if (isMultiValue) {
                    return getNumberPredicate(path, operator, Stream.of(value.split(multivalueSeperator)).map(Long::parseLong).toArray(Long[]::new));
                } else {
            ...
            case "Double": {
                ...
                if (isMultiValue) {
                    return getNumberPredicate(path, operator, Stream.of(value.split(multivalueSeperator)).map(Double::parseDouble).toArray(Double[]::new));
                } else {
     ...
     * @return
     */
    private <N extends Number & Comparable<?>> BooleanExpression getNumberPredicate(NumberPath<N> path, SearchOperator operator, N value) {
     ...
     * @return
     */
    private BooleanExpression getNumberPredicate(NumberPath<?> path, SearchOperator operator, Object[] numValues) {
     ...
     */
    private BooleanExpression getDatePredicate(String key, SearchOperator operator, String value, PathBuilder<?> entityPath) {
     ...
     */
    private BooleanExpression getDateTimePredicate(String key, SearchOperator operator, String value, PathBuilder<?> entityPath) throws Exception {
        DatePath<LocalDateTime> path = entityPath.getDate(key, LocalDateTime.class);
        if (value.contains(",")) {
            List<LocalDate> dateTimeValues = Stream.of(value.split(",")).map(LocalDate::parse).collect(Collectors.toList());
            return switch (operator) {
                case EQUALS -> path.in(dateTimeValues.parallelStream().map(LocalDate::atStartOfDay).collect(Collectors.toList()));
                case BETWEEN -> path.between(dateTimeValues.get(0).atStartOfDay(), dateTimeValues.get(1).atTime(LocalTime.MAX));
                default -> null;
            };
        } else {
            LocalDate dateValue = LocalDate.parse(value);
     ...
     */
    private BooleanExpression getStringPredicate(String key, SearchOperator operator, String value, PathBuilder<?> entityPath) throws Exception {
