...
public abstract class FullAuditedCrudServiceImpl<
        D extends EntityDto<ID>, //đối tượng dto service cần phản hồi
        T extends IEntity<ID> & FullAudited<UID>, // đối tượng service thực sự
        ID extends Comparable<ID> & Serializable, // kiểu dữ liệu của đối tượng
        U extends EntityDto<UID>, // cái đối tượng thực hiện các service này
        UID extends Comparable<UID> & Serializable> // kiểu dữ liệu của đối tượng thực hiện hành động đó

        extends AuditedCrudServiceImpl<D, T, ID, U, UID> implements FullAuditedCrudService<D, T, ID, U, UID> {
