...
@Transactional
public abstract class CreationAuditedCrudServiceImpl<D extends EntityDto<ID>, T extends IEntity<ID> & CreationAudited<UID>, ID extends Comparable<ID> & Serializable, U extends EntityDto<UID>, UID extends Comparable<UID> & Serializable>
        extends CreationAuditedGetServiceImpl<D, T, ID, U, UID> implements CreationAuditedCrudService<D, T, ID, U, UID> {
    ...
    protected D createEntity(T entity) {
        ...
        getLogger().info("Created entity id {} by user {}: {}", entity.getId(), entity.getCreatorId(), entity);
    ...
    public D create(CreateInput<T> createDto, U user) throws AppException {
        ...
        if (entity instanceof CreationAuditedSerialIdEntity || entity instanceof CreationAuditedGeneratedIdEntity) {
            entity.setId(null);
        }
