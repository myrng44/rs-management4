...
@Getter
public abstract class EntityCache<CachedItem extends EntityDto<ID>, ID extends Comparable<ID> & Serializable> {
    ...
    public CachedItem put(ID id, CachedItem cachedItem) {
        getLogger().info("[CACHE] {} - Put entity with id {} to cache: {}", getCacheName(), id, cachedItem);
        return cachedItem;
    }
